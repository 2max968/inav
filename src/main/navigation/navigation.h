...
PG_DECLARE_ARRAY(navFwAutolandApproach_t, MAX_FW_LAND_APPOACH_SETTINGS,
...
bool geozoneSetVertex(uint8_t zoneId, uint8_t vertexId, int32_t lat,
                      int32_t lon);
...
#define NAV_ACCEL_CUTOFF_FREQUENCY_HZ                                          \
  2 // low-pass filter on XY-acceleration target
...
enum {
  NAV_RTH_CLIMB_STAGE_AT_LEAST =
      0, // Will climb to the lesser of rth_climb_first_stage_altitude or
         // rth_altitude, before turning
  NAV_RTH_CLIMB_STAGE_EXTRA =
      1, // Will climb the lesser of rth_climb_first_stage_altitude above the
         // current altitude or to nav_rth_altitude, before turning
};
...
typedef struct positionEstimationConfig_s {
  ...
  uint8_t gravity_calibration_tolerance; // Tolerance of gravity calibration
                                         // (cm/s/s)
  uint8_t allow_dead_reckoning;

  uint16_t max_surface_altitude;

  float w_z_baro_p; // Weight (cutoff frequency) for barometer altitude
                    // measurements
  float w_z_baro_v; // Weight (cutoff frequency) for barometer climb rate
                    // measurements

  float w_z_surface_p; // Weight (cutoff frequency) for surface altitude
                       // measurements
  float w_z_surface_v; // Weight (cutoff frequency) for surface velocity
                       // measurements

  float w_z_gps_p; // GPS altitude data is very noisy and should be used only on
                   // airplanes
  ...
  float w_z_res_v; // When velocity sources lost slowly decrease estimated
                   // velocity with this weight
  float w_xy_res_v;

  float w_acc_bias; // Weight (cutoff frequency) for accelerometer bias
                    // estimation. 0 to disable.

  float max_eph_epv; // Max estimated position error acceptable for estimation
                     // (cm)
...
typedef struct navConfig_s {
  ...
  struct {
    struct {
      ...
      uint8_t rth_alt_control_mode; // Controls the logic for choosing the RTH
                                    // altitude
      uint8_t rth_climb_first; // Controls the logic for initial RTH climbout
      uint8_t
          rth_climb_first_stage_mode; // To determine how
                                      // rth_climb_first_stage_altitude is used
      uint8_t rth_tail_first;         // Return to home tail first
      uint8_t disarm_on_landing;      //
      uint8_t rth_allow_landing; // Enable landing as last stage of RTH. Use
                                 // constants in navRTHAllowLanding_e.
      uint8_t rth_climb_ignore_emerg;   // Option to ignore GPS loss on initial
                                        // climb stage of RTH
      uint8_t rth_alt_control_override; // Override RTH Altitude and Climb First
                                        // settings using Pitch and Roll stick
      uint8_t
          nav_overrides_motor_stop;  // Autonomous modes override motor_stop
                                     // setting and user command to stop motor
      ...
      uint8_t mission_planner_reset; // Allow WP Mission Planner reset using
                                     // mode toggle (resets WPs to 0)
      ...
      uint8_t
          rth_use_linear_descent; // Use linear descent in the RTH head home leg
      uint8_t landing_bump_detection; // Allow landing detection based on G bump
                                      // at touchdown
    } flags;

    uint8_t pos_failure_timeout; // Time to wait before switching to emergency
                                 // landing (0 - disable)
    uint16_t waypoint_radius;    // if we are within this distance to a waypoint
                              // then we consider it reached (distance is in cm)
    uint16_t waypoint_safe_distance; // Waypoint mission sanity check distance
#ifdef USE_MULTI_MISSION
    uint8_t waypoint_multi_mission_index; // Index of mission to be loaded in
                                          // multi mission entry
#endif
    ...
    uint16_t
        max_auto_speed; // maximum allowed autonomous navigation speed cm/sec
    ...
    uint16_t land_slowdown_minalt; // Altitude to stop lowering descent rate
                                   // during RTH descend
    uint16_t land_slowdown_maxalt; // Altitude to start lowering descent rate
                                   // during RTH descend
    uint16_t emerg_descent_rate;   // emergency landing descent rate
    uint16_t rth_altitude; // altitude to maintain when RTH is active (depends
                           // on rth_alt_control_mode) (cm)
    uint16_t rth_home_altitude; // altitude to go to during RTH after the craft
                                // reached home (cm)
    uint16_t rth_climb_first_stage_altitude; // Altitude to reach before
                                             // transitioning from climb first
                                             // to turn first
    uint16_t min_rth_distance;    // 0 Disables. Minimal distance for RTH in cm,
                                  // otherwise it will just autoland
    uint16_t rth_abort_threshold; // Initiate emergency landing if during RTH we
                                  // get this much [cm] away from home
    uint16_t max_terrain_follow_altitude; // Max altitude to be used in SURFACE
                                          // TRACKING mode
    uint16_t safehome_max_distance; // Max distance that a safehome is from the
                                    // arming point
    uint16_t max_altitude; // Max altitude when in AltHold mode (not Surface
                           // Following)
    uint16_t rth_trackback_distance; // RTH trackback maximum distance [m]
    uint16_t
        waypoint_enforce_altitude;   // Forces waypoint altitude to be achieved
    ...
    uint16_t
        rth_linear_descent_start_distance; // Distance from home to start the
                                           // linear descent (0 = immediately)
    uint8_t cruise_yaw_rate; // Max yaw rate (dps) when CRUISE MODE is enabled
    uint16_t rth_fs_landing_delay; // Delay upon reaching home before starting
                                   // landing if in FS (0 = immediate)
  } general;

  struct {
    uint8_t max_bank_angle; // multicopter max banking angle (deg)
    uint16_t
        max_auto_climb_rate; // max vertical speed limitation nav modes cm/sec
    uint16_t
        max_manual_climb_rate; // manual velocity control max vertical speed

#ifdef USE_MR_BRAKING_MODE
    uint16_t braking_speed_threshold; // above this speed braking routine might
                                      // kick in
    uint16_t
        braking_disengage_speed;  // below this speed braking will be disengaged
    ...
    uint16_t braking_boost_speed_threshold; // Above this speed braking boost
                                            // mode can engage
    uint16_t braking_boost_disengage_speed; // Below this speed braking boost
                                            // will disengage
    uint8_t braking_bank_angle; // Max angle [deg] that MR is allowed duing
                                // braking boost phase
#endif

    uint8_t posDecelerationTime; // Brake time parameter
    uint8_t posResponseExpo; // Position controller expo (taret vel expo for MC)
    bool slowDownForTurning; // Slow down during WP missions when changing
                             // heading on next waypoint
    uint8_t althold_throttle_type; // throttle zero datum type for alt hold
    uint8_t
        inverted_crash_detection; // Enables inverted crash detection, setting
                                  // defines disarm time delay (0 = disabled)
  } mc;
  ...
  struct {
    ...
    uint16_t cruise_speed;      // Speed at cruise throttle (cm/s), used for
                                // time/distance left before RTH
    uint8_t control_smoothness; // The amount of smoothing to apply to controls
                                // for navigation
    uint16_t
        pitch_to_throttle_smooth; // How smoothly the autopilot makes pitch to
                                  // throttle correction inside a deadband
                                  // defined by pitch_to_throttle_thresh.
    uint8_t pitch_to_throttle_thresh; // Threshold from average pitch where
                                      // momentary pitch_to_throttle correction
                                      // kicks in. [decidegrees]
    uint16_t
        minThrottleDownPitchAngle; // Automatic pitch down angle when throttle
                                   // is at 0 in angle mode. Progressively
                                   // applied between cruise throttle and zero
                                   // throttle. [decidegrees]
    uint16_t loiter_radius;   // Loiter radius when executing PH on a fixed wing
    uint8_t loiter_direction; // Direction of loitering center point on right
                              // wing (clockwise - as before), or center point
                              // on left wing (counterclockwise)
    int8_t land_dive_angle;
    uint16_t
        launch_velocity_thresh; // Velocity threshold for swing launch detection
    uint16_t launch_accel_thresh; // Acceleration threshold for launch detection
                                  // (cm/s/s)
    uint16_t launch_time_thresh;  // Time threshold for launch detection (ms)
    uint16_t
        launch_motor_timer; // Time to wait before setting launch_throttle (ms)
    uint16_t launch_idle_motor_timer; // Time to wait before motor starts
                                      // at_idle throttle (ms)
    uint8_t launch_wiggle_wake_idle;  // Activate the idle throttle by wiggling
                                      // the plane. 0 = disabled, 1 or 2 specify
                                      // the number of wiggles.
    uint16_t
        launch_motor_spinup_time; // Time to speed-up motors from idle to
                                  // launch_throttle (ESC desync prevention)
    uint16_t launch_end_time; // Time to make the transition from launch angle
                              // to leveled and throttle transition from launch
                              // throttle to the stick position
    uint16_t
        launch_min_time; // Minimum time in launch mode to prevent possible bump
                         // of the sticks from leaving launch mode early
    uint16_t launch_timeout; // Launch timeout to disable launch mode and swith
                             // to normal flight (ms)
    uint16_t launch_max_altitude; // cm, altitude where to consider launch ended
    uint8_t launch_climb_angle;   // Target climb angle for launch (deg)
    uint8_t launch_max_angle;     // Max tilt angle (pitch/roll combined) to
                                  // consider launch successful. Set to 180 to
                                  // disable completely [deg]
    bool launch_manual_throttle;  // Allows launch with manual throttle control
    uint8_t launch_land_abort_deadband; // roll/pitch stick movement deadband
                                        // for launch abort
    uint8_t cruise_yaw_rate; // Max yaw rate (dps) when CRUISE MODE is enabled
    ...
    uint8_t
        wp_tracking_accuracy; // fixed wing tracking accuracy response factor
    uint8_t wp_tracking_max_angle; // fixed wing tracking accuracy max alignment
                                   // angle [degs]
...
typedef enum {
  ...
  MW_NAV_ERROR_SPOILED_GPS, // GPS reception is compromised - Nav paused -
                            // copter is adrift !
...
/* Geodetic functions */
typedef enum { GEO_ALT_ABSOLUTE, GEO_ALT_RELATIVE } geoAltitudeConversionMode_e;

typedef enum { GEO_ORIGIN_SET, GEO_ORIGIN_RESET_ALTITUDE } geoOriginResetMode_e;

typedef enum { NAV_WP_TAKEOFF_DATUM, NAV_WP_MSL_DATUM } geoAltitudeDatumFlag_e;
...
void geoSetOrigin(gpsOrigin_t *origin, const gpsLocation_t *llh,
...
// default GPS origin.
bool geoConvertGeodeticToLocalOrigin(fpVector3_t *pos, const gpsLocation_t *llh,
                                     geoAltitudeConversionMode_e altConv);
...
bool geoConvertLocalToGeodetic(gpsLocation_t *llh, const gpsOrigin_t *origin,
...
bool isWaypointReached(const fpVector3_t *waypointPos,
                       const int32_t *waypointBearing);

/* Distance/bearing calculation */
bool navCalculatePathToDestination(
    navDestinationPath_t *result,
    const fpVector3_t *destinationPos); // NOT USED
...
/* Returns true if navConfig()->general.flags.rth_allow_landing is
 * NAV_RTH_ALLOW_LANDING_ALWAYS or if it's NAV_RTH_ALLOW_LANDING_FAILSAFE and
