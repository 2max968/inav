...
#define INAV_GPS_DEFAULT_EPH                                                   \
  200.0f // 2m GPS HDOP  (gives about 1.6s of dead-reckoning if GPS is temporary
         // lost)
...
#define INAV_ACC_BIAS_ACCEPTANCE_VALUE                                         \
  (GRAVITY_CMSS * 0.25f) // Max accepted bias correction of 0.25G - unlikely we
                         // are going to be that much off anyway

#define INAV_GPS_GLITCH_RADIUS 250.0f // 2.5m GPS glitch radius
#define INAV_GPS_GLITCH_ACCEL                                                  \
  1000.0f // 10m/s/s max possible acceleration for GPS glitch detection

#define INAV_POSITION_PUBLISH_RATE_HZ                                          \
  50 // Publish position updates at this rate
#define INAV_PITOT_UPDATE_RATE 10

#define INAV_GPS_TIMEOUT_MS 1500 // GPS timeout
#define INAV_BARO_TIMEOUT_MS 200 // Baro timeout
#define INAV_SURFACE_TIMEOUT_MS                                                \
  400 // Surface timeout    (missed 3 readings in a row)
#define INAV_FLOW_TIMEOUT_MS 200

#define CALIBRATING_GRAVITY_TIME_MS 2000

// Time constants for calculating Baro/Sonar averages. Should be the same value
// to impose same amount of group delay
#define INAV_BARO_AVERAGE_HZ 1.0f
#define INAV_SURFACE_AVERAGE_HZ 1.0f

#define INAV_ACC_CLIPPING_RC_CONSTANT                                          \
  (0.010f) // Reduce acc weight for ~10ms after clipping
...
typedef enum {
  SURFACE_QUAL_LOW, // Surface sensor signal lost long ago - most likely surface
                    // distance is incorrect
  SURFACE_QUAL_MID, // Surface sensor is not available but we can somewhat trust
                    // the estimate
...
typedef struct {
  ...
  float altRaw; // Raw altitude measurement (cm) - Unfiltered/Unchecked
  float altRawLast;
  float altOffset;
  float reliability;
...
extern float updateEPE(const float oldEPE, const float dt, const float newEPE,
                       const float w);
