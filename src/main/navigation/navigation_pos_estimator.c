...
PG_RESET_TEMPLATE(
    ...
    .gravity_calibration_tolerance =
        SETTING_INAV_GRAVITY_CAL_TOLERANCE_DEFAULT, // 5 cm/s/s calibration
                                                    // error accepted (0.5% of
                                                    // gravity)
...
static bool updateTimer(navigationTimer_t *tim, timeUs_t interval,
...
static bool shouldResetReferenceAltitude(void) {
   ...
   * If emerg in flight rearm active ref altitudes reset to backup values to
   * avoid unwanted altitude reset */
...
#if defined(USE_GPS)
/* Why is this here: Because GPS will be sending at quiet a nailed rate (if not
 * overloaded by junk tasks at the brink of its specs) but we might read out
 * with timejitter because Irq might be off by a few us so we do a +-10% margin
 * around the time between GPS datasets representing the most common Hz-rates
 * today. You might want to extend the list or find a smarter way. Don't
 * overload your GPS in its config with trash, choose a Hz rate that it can
 * deliver at a sustained rate. (c) CrashPilot1000
 */
static timeUs_t getGPSDeltaTimeFilter(timeUs_t dTus) {
  if (dTus >= 225000 && dTus <= 275000)
    return HZ2US(4); //  4Hz Data 250ms
  if (dTus >= 180000 && dTus <= 220000)
    return HZ2US(5); //  5Hz Data 200ms
  if (dTus >= 90000 && dTus <= 110000)
    return HZ2US(10); // 10Hz Data 100ms
  if (dTus >= 45000 && dTus <= 55000)
    return HZ2US(20); // 20Hz Data  50ms
  if (dTus >= 30000 && dTus <= 36000)
    return HZ2US(30); // 30Hz Data  33ms
  if (dTus >= 23000 && dTus <= 27000)
    return HZ2US(40); // 40Hz Data  25ms
  if (dTus >= 18000 && dTus <= 22000)
    return HZ2US(50); // 50Hz Data  20ms
...
void onNewGPSData(void) {
  ...
  if (sensors(SENSOR_GPS)
    ...
    /* Process position update if GPS origin is already set, or precision is
     * good enough */
    // FIXME: Add HDOP check for acquisition of GPS origin
    /* Set GPS origin or reset the origin altitude - keep initial pre-arming
     * altitude at zero */
    ...
    if (posControl.gpsOrigin.valid) {
      ...
      geoConvertGeodeticToLocal(&posEstimator.gps.pos, &posControl.gpsOrigin,
                                &newLLH, GEO_ALT_ABSOLUTE);

      /* If not the first update - calculate velocities */
      if (!isFirstGPSUpdate) {
        float dT =
            US2S(getGPSDeltaTimeFilter(currentTimeUs - lastGPSNewDataTime));

        /* Use VELNED provided by GPS if available, calculate from coordinates
         * otherwise */
        float gpsScaleLonDown = constrainf(
            cos_approx((ABS(gpsSol.llh.lat) / 10000000.0f) * 0.0174532925f),
            0.01f, 1.0f);
        if (!ARMING_FLAG(SIMULATOR_MODE_SITL) && gpsSol.flags.validVelNE) {
          ...
          posEstimator.gps.vel.x =
              (posEstimator.gps.vel.x +
               (DISTANCE_BETWEEN_TWO_LONGITUDE_POINTS_AT_EQUATOR *
                (gpsSol.llh.lat - previousLat) / dT)) /
              2.0f;
          posEstimator.gps.vel.y =
              (posEstimator.gps.vel.y +
               (gpsScaleLonDown *
                DISTANCE_BETWEEN_TWO_LONGITUDE_POINTS_AT_EQUATOR *
                (gpsSol.llh.lon - previousLon) / dT)) /
              2.0f;
        }
        ...
        if (gpsSol.flags.validVelD) {
          ...
          posEstimator.gps.vel.z =
              (posEstimator.gps.vel.z + (gpsSol.llh.alt - previousAlt) / dT) /
              2.0f;
        }
...
void updatePositionEstimator_BaroTopic(timeUs_t currentTimeUs) {
  ...
  if (sensors(SENSOR_BARO) && baroIsCalibrationComplete()) {
    ...
    if (baroDtUs <= MS2US(INAV_BARO_TIMEOUT_MS)) {
      posEstimator.baro.alt = pt1FilterApply3(
          &posEstimator.baro.avgFilter, posEstimator.baro.alt, US2S(baroDtUs));

      // baro altitude rate
      static float baroAltPrevious = 0;
      posEstimator.baro.baroAltRate =
          (posEstimator.baro.alt - baroAltPrevious) / US2S(baroDtUs);
...
static void restartGravityCalibration(void) {
  ...
  zeroCalibrationStartS(
      &posEstimator.imu.gravityCalibration, CALIBRATING_GRAVITY_TIME_MS,
      positionEstimationConfig()->gravity_calibration_tolerance, false);
}
...
static void updateIMUEstimationWeight(const float dt) {
  ...
  float acc_vibration_factor = scaleRangef(
      constrainf(accGetVibrationLevel(), ACC_VIB_FACTOR_S, ACC_VIB_FACTOR_E),
      ACC_VIB_FACTOR_S, ACC_VIB_FACTOR_E, 1.0f, 0.3f); // g/s
...
static void updateIMUTopic(timeUs_t currentTimeUs) {
  ...
  if (!isImuReady()) {
    ...
    /* Adjust reading from Body to Earth frame - from Forward-Right-Down to
     * North-East-Up*/
    ...
    /* When unarmed, assume that accelerometer should measure 1G. Use that to
     * correct accelerometer gain */
    if (gyroConfig()->init_gyro_cal_enabled) {
      if (!ARMING_FLAG(ARMED) && !gravityCalibrationComplete()) {
        zeroCalibrationAddValueS(&posEstimator.imu.gravityCalibration,
                                 posEstimator.imu.accelNEU.z);

        if (gravityCalibrationComplete()) {
          zeroCalibrationGetZeroS(&posEstimator.imu.gravityCalibration,
                                  &posEstimator.imu.calibratedGravityCMSS);
          setGravityCalibration(posEstimator.imu.calibratedGravityCMSS);
          LOG_DEBUG(POS_ESTIMATOR, "Gravity calibration complete (%d)",
                    (int)lrintf(posEstimator.imu.calibratedGravityCMSS));
        }
      }
    } else {
    ...
    if (gravityCalibrationComplete()) {
      ...
      posEstimator.imu.accelNEU.z += applySensorTempCompensation(
          10 * gyroGetTemperature(), imuMeasuredAccelBF.z, SENSOR_INDEX_ACC);
...
float updateEPE(const float oldEPE, const float dt, const float newEPE,
                const float w) {
  return oldEPE + (newEPE - oldEPE) * w * dt;
}

static bool navIsAccelerationUsable(void) { return true; }
...
static bool navIsHeadingUsable(void) {
  if (sensors(SENSOR_GPS)
    ...
    // If we don't have GPS - we may use whatever we have, other sensors are
    // operating in body frame
    return isImuHeadingValid() ||
...
static uint32_t calculateCurrentValidityFlags(timeUs_t currentTimeUs) {
  ...
  if ((sensors(SENSOR_GPS)
      ...
      ((currentTimeUs - posEstimator.gps.lastUpdateTime) <=
  ...
  if (sensors(SENSOR_BARO) &&
      ((currentTimeUs - posEstimator.baro.lastUpdateTime) <=
       MS2US(INAV_BARO_TIMEOUT_MS))) {
    newFlags |= EST_BARO_VALID;
  }

  if (sensors(SENSOR_RANGEFINDER) &&
      ((currentTimeUs - posEstimator.surface.lastUpdateTime) <=
       MS2US(INAV_SURFACE_TIMEOUT_MS))) {
    newFlags |= EST_SURFACE_VALID;
  }

  if (sensors(SENSOR_OPFLOW) && posEstimator.flow.isValid &&
      ((currentTimeUs - posEstimator.flow.lastUpdateTime) <=
...
static void estimationPredict(estimationContext_t *ctx) {
  ...
  if ((ctx->newFlags & EST_XY_VALID)) {
    ...
    if (navIsHeadingUsable() && navIsAccelerationUsable()) {
      posEstimator.est.pos.x +=
          posEstimator.imu.accelNEU.x * sq(ctx->dt) / 2.0f;
      posEstimator.est.pos.y +=
          posEstimator.imu.accelNEU.y * sq(ctx->dt) / 2.0f;
...
static bool estimationCalculateCorrection_Z(estimationContext_t *ctx) {
  ...
  DEBUG_SET(DEBUG_ALTITUDE, 1,
            posEstimator.est.vel.z); // Vertical speed estimate
  DEBUG_SET(
      DEBUG_ALTITUDE, 3,
      posEstimator.imu.accelNEU.z); // Vertical acceleration on earth frame
  DEBUG_SET(DEBUG_ALTITUDE, 5, posEstimator.gps.vel.z); // GPS vertical speed
  DEBUG_SET(DEBUG_ALTITUDE, 7, accGetClipCount());      // Clip count

  bool correctOK = false;
  const uint8_t defaultAltitudeSource =
      positionEstimationConfig()->default_alt_sensor;
  float wGps = defaultAltitudeSource == ALTITUDE_SOURCE_BARO_ONLY &&
                       ctx->newFlags & EST_BARO_VALID
                   ? 0.0f
                   : 1.0f;
  float wBaro = defaultAltitudeSource == ALTITUDE_SOURCE_GPS_ONLY &&
                        ctx->newFlags & EST_GPS_Z_VALID
                    ? 0.0f
                    : 1.0f;

  if (wBaro && ctx->newFlags & EST_BARO_VALID && wGps &&
      ctx->newFlags & EST_GPS_Z_VALID) {
    const float gpsBaroResidual =
        fabsf(posEstimator.gps.pos.z - posEstimator.baro.alt);

    // Fade out the non default sensor to prevent sudden jump
    uint16_t residualErrorEpvLimit =
        defaultAltitudeSource == ALTITUDE_SOURCE_BARO
            ? 2 * positionEstimationConfig()->baro_epv
    ...
    // Calculate residual gps/baro sensor weighting based on assumed default
    // altitude source = GPS
    wBaro = scaleRangef(constrainf(gpsBaroResidual, start_epv, end_epv),
                        start_epv, end_epv, 1.0f, 0.0f);

    if (defaultAltitudeSource ==
        ALTITUDE_SOURCE_BARO) { // flip residual sensor weighting if default =
                                // BARO
      wGps = wBaro;
      wBaro = 1.0f;
    }
  ...
  if (ctx->newFlags & EST_BARO_VALID && wBaro) {
    ...
    if (STATE(MULTIROTOR)) {
      ...
      if (!ARMING_FLAG(ARMED)) {
        ...
        // We might be experiencing air cushion effect during takeoff - use
        // sonar or baro ground altitude to detect it
        if (isMulticopterThrottleAboveMidHover()) {
          // Disable ground effect detection at lift off when est alt and baro
          // alt converge. Always disable if baro alt > 1m.
          isBaroGroundValid =
              fabsf(posEstimator.est.pos.z - posEstimator.baro.alt) > 20.0f &&
              posEstimator.baro.alt < 100.0f;
        }

        isAirCushionEffectDetected =
            (isEstimatedAglTrusted() && posEstimator.surface.alt < 20.0f) ||
    ...
    float baroAltResidual =
        wBaro *
        ((isAirCushionEffectDetected ? baroGroundAlt : posEstimator.baro.alt) -
    ...
    const float baroVelZResidual =
        isAirCushionEffectDetected
            ? 0.0f
            : wBaro * (posEstimator.baro.baroAltRate - posEstimator.est.vel.z);
    ...
    ctx->newEPV = updateEPE(posEstimator.est.epv, ctx->dt,
                            MAX(posEstimator.baro.epv, fabsf(baroAltResidual)),
                            w_z_baro_p);

    // Accelerometer bias
    if (!isAirCushionEffectDetected) {
      ctx->accBiasCorr.z += (baroAltResidual * sq(w_z_baro_p) +
                             baroVelZResidual * sq(w_z_baro_v));
    }
  ...
  if (ctx->newFlags & EST_GPS_Z_VALID &&
      (wGps || !(ctx->newFlags & EST_Z_VALID))) {
    // Reset current estimate to GPS altitude if estimate not valid (used for
    // GPS and Baro)
    if (!(ctx->newFlags & EST_Z_VALID)) {
      ...
      // Altitude
      const float gpsAltResidual =
          wGps * (posEstimator.gps.pos.z - posEstimator.est.pos.z);
      const float gpsVelZResidual =
          wGps * (posEstimator.gps.vel.z - posEstimator.est.vel.z);
      ...
      ctx->newEPV = updateEPE(posEstimator.est.epv, ctx->dt,
                              MAX(posEstimator.gps.epv, fabsf(gpsAltResidual)),
                              w_z_gps_p);

      // Accelerometer bias
      ctx->accBiasCorr.z +=
          (gpsAltResidual * sq(w_z_gps_p) + gpsVelZResidual * sq(w_z_gps_v));
    }
...
static bool estimationCalculateCorrection_XY_GPS(estimationContext_t *ctx) {
  if (ctx->newFlags & EST_GPS_XY_VALID) {
    /* If GPS is valid and our estimate is NOT valid - reset it to GPS
    ...
    if (!(ctx->newFlags & EST_XY_VALID)) {
      ...
      const float gpsPosXResidual =
          posEstimator.gps.pos.x - posEstimator.est.pos.x;
      const float gpsPosYResidual =
          posEstimator.gps.pos.y - posEstimator.est.pos.y;
      const float gpsVelXResidual =
          posEstimator.gps.vel.x - posEstimator.est.vel.x;
      const float gpsVelYResidual =
          posEstimator.gps.vel.y - posEstimator.est.vel.y;
      const float gpsPosResidualMag =
          calc_length_pythagorean_2D(gpsPosXResidual, gpsPosYResidual);
      ...
      const float w_xy_gps_p =
          positionEstimationConfig()->w_xy_gps_p * gpsWeightScaler;
      const float w_xy_gps_v =
          positionEstimationConfig()->w_xy_gps_v * sq(gpsWeightScaler);
      ...
      // Accelerometer bias
      ctx->accBiasCorr.x +=
          (gpsPosXResidual * sq(w_xy_gps_p) + gpsVelXResidual * sq(w_xy_gps_v));
      ctx->accBiasCorr.y +=
          (gpsPosYResidual * sq(w_xy_gps_p) + gpsVelYResidual * sq(w_xy_gps_v));

      /* Adjust EPH */
      ctx->newEPH =
          updateEPE(posEstimator.est.eph, ctx->dt,
                    MAX(posEstimator.gps.eph, gpsPosResidualMag), w_xy_gps_p);
    }
...
static void estimationCalculateGroundCourse(timeUs_t currentTimeUs) {
  ...
  if ((STATE(GPS_FIX)
      ...
      navIsHeadingUsable()) {
    uint32_t groundCourse = wrap_36000(RADIANS_TO_CENTIDEGREES(
        atan2_approx(posEstimator.est.vel.y, posEstimator.est.vel.x)));
    posEstimator.est.cog = CENTIDEGREES_TO_DECIDEGREES(groundCourse);
  }
}

/**
 * Calculate next estimate using IMU and apply corrections from reference
 * sensors (GPS, BARO etc) Function is called at main loop rate
 */
static void updateEstimatedTopic(timeUs_t currentTimeUs) {
  ...
  ctx.newEPH = posEstimator.est.eph *
               ((posEstimator.est.eph <= max_eph_epv) ? 1.0f + ctx.dt : 1.0f);
  ctx.newEPV = posEstimator.est.epv *
               ((posEstimator.est.epv <= max_eph_epv) ? 1.0f + ctx.dt : 1.0f);
  ...
  // FIXME: Handle transition from FLOW to GPS and back - seamlessly fly
  // indoor/outdoor
  const bool estXYCorrectOk = estimationCalculateCorrection_XY_GPS(&ctx) ||
                              estimationCalculateCorrection_XY_FLOW(&ctx);

  // If we can't apply correction or accuracy is off the charts - decay velocity
  // to zero
  if (!estXYCorrectOk || ctx.newEPH > max_eph_epv) {
    ctx.estVelCorr.x = (0.0f - posEstimator.est.vel.x) *
                       positionEstimationConfig()->w_xy_res_v * ctx.dt;
    ctx.estVelCorr.y = (0.0f - posEstimator.est.vel.y) *
                       positionEstimationConfig()->w_xy_res_v * ctx.dt;
  }

  if (!estZCorrectOk || ctx.newEPV > max_eph_epv) {
    ctx.estVelCorr.z = (0.0f - posEstimator.est.vel.z) *
                       positionEstimationConfig()->w_z_res_v * ctx.dt;
  }
  // Boost the corrections based on accWeight
  vectorScale(&ctx.estPosCorr, &ctx.estPosCorr,
              1.0f / posEstimator.imu.accWeightFactor);
  vectorScale(&ctx.estVelCorr, &ctx.estVelCorr,
              1.0f / posEstimator.imu.accWeightFactor);
  ...
  if (w_acc_bias > 0.0f) {
    ...
    posEstimator.imu.accelBias.x = constrainf(posEstimator.imu.accelBias.x,
                                              -INAV_ACC_BIAS_ACCEPTANCE_VALUE,
                                              INAV_ACC_BIAS_ACCEPTANCE_VALUE);
    posEstimator.imu.accelBias.y = constrainf(posEstimator.imu.accelBias.y,
                                              -INAV_ACC_BIAS_ACCEPTANCE_VALUE,
                                              INAV_ACC_BIAS_ACCEPTANCE_VALUE);
    posEstimator.imu.accelBias.z = constrainf(posEstimator.imu.accelBias.z,
                                              -INAV_ACC_BIAS_ACCEPTANCE_VALUE,
                                              INAV_ACC_BIAS_ACCEPTANCE_VALUE);
  }
...
/**
 * Examine estimation error and update navigation system if estimate is good
 * enough Function is called at main loop rate, but updates happen less
 * frequently - at a fixed rate
 */
static void publishEstimatedTopic(timeUs_t currentTimeUs) {
  ...
  if (updateTimer(&posPublishTimer, HZ2US(INAV_POSITION_PUBLISH_RATE_HZ),
                  currentTimeUs)) {
    ...
    int16_t cogValue =
        isGPSHeadingValid() ? posEstimator.est.cog : attitude.values.yaw;
    updateActualHeading(navIsHeadingUsable(),
    ...
    if (posEstimator.est.eph < positionEstimationConfig()->max_eph_epv) {
      ...
      updateActualHorizontalPositionAndVelocity(
          true, true, posEstimator.est.pos.x, posEstimator.est.pos.y,
          posEstimator.est.vel.x, posEstimator.est.vel.y);
    } else {
      updateActualHorizontalPositionAndVelocity(
          false, false, posEstimator.est.pos.x, posEstimator.est.pos.y, 0, 0);
    }

    /* Publish altitude update and set altitude validity */
    if (posEstimator.est.epv < positionEstimationConfig()->max_eph_epv) {
      const float gpsCfEstimatedAltitudeError =
          STATE(GPS_FIX) ? posEstimator.gps.pos.z - posEstimator.est.pos.z : 0;
      navigationEstimateStatus_e aglStatus =
          (posEstimator.est.aglQual == SURFACE_QUAL_LOW) ? EST_USABLE
                                                         : EST_TRUSTED;
      updateActualAltitudeAndClimbRate(
          true, posEstimator.est.pos.z, posEstimator.est.vel.z,
          posEstimator.est.aglAlt, posEstimator.est.aglVel, aglStatus,
          gpsCfEstimatedAltitudeError);
    } else {
      updateActualAltitudeAndClimbRate(false, posEstimator.est.pos.z, 0,
                                       posEstimator.est.aglAlt, 0, EST_NONE, 0);
    }
    ...
    DEBUG_SET(DEBUG_POS_EST, 0,
              (int32_t)posEstimator.est.pos.x * 1000.0F); // Position estimate X
    DEBUG_SET(DEBUG_POS_EST, 1,
              (int32_t)posEstimator.est.pos.y * 1000.0F); // Position estimate Y
    if (IS_RC_MODE_ACTIVE(BOXSURFACE) &&
        posEstimator.est.aglQual != SURFACE_QUAL_LOW) {
      // SURFACE (following) MODE
      DEBUG_SET(DEBUG_POS_EST, 2,
                (int32_t)posControl.actualState.agl.pos.z *
                    1000.0F); // Position estimate Z
      DEBUG_SET(DEBUG_POS_EST, 5,
                (int32_t)posControl.actualState.agl.vel.z *
                    1000.0F); // Speed estimate VZ
    } else {
      DEBUG_SET(DEBUG_POS_EST, 2,
                (int32_t)posEstimator.est.pos.z *
                    1000.0F); // Position estimate Z
      DEBUG_SET(DEBUG_POS_EST, 5,
                (int32_t)posEstimator.est.vel.z * 1000.0F); // Speed estimate VZ
    }
    DEBUG_SET(DEBUG_POS_EST, 3,
              (int32_t)posEstimator.est.vel.x * 1000.0F); // Speed estimate VX
    DEBUG_SET(DEBUG_POS_EST, 4,
              (int32_t)posEstimator.est.vel.y * 1000.0F); // Speed estimate VY
    DEBUG_SET(DEBUG_POS_EST, 6,
              (int32_t)attitude.values
                  .yaw); // Yaw estimate (4 bytes still available here)
    DEBUG_SET(DEBUG_POS_EST, 7,
              (int32_t)(posEstimator.flags & 0b1111111)
                      << 20 | // navPositionEstimationFlags fit into 8bits
                  (MIN(navEPH, 1000) & 0x3FF) << 10 |
                  (MIN(navEPV, 1000) &
                   0x3FF)); // Horizontal and vertical uncertainties (max value
                            // = 1000, fit into 20bits)
  }
}

float getEstimatedAglPosition(void) { return posEstimator.est.aglAlt; }
...
bool navIsCalibrationComplete(void) { return gravityCalibrationComplete(); }
