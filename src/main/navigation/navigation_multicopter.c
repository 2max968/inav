...
float getSqrtControllerVelocity(float targetAltitude, timeDelta_t deltaMicros) {
  return sqrtControllerApply(&alt_hold_sqrt_controller, targetAltitude,
...
static void updateAltitudeVelocityController_MC(timeDelta_t deltaMicros) {
  float targetVel =
      getDesiredClimbRate(posControl.desiredState.pos.z, deltaMicros);
  ...
  if (velTargetChange <= -smallVelChange) {
    // Large & Negative - acceleration is _down_. We can't reach more than -1G
    // in any possible condition. Hard limit to 0.8G to stay safe This should be
    // safe enough for stability since we only reduce throttle
    const float maxVelDifference = US2S(deltaMicros) * (GRAVITY_CMSS * 0.8f);
    posControl.desiredState.vel.z =
        constrainf(targetVel, posControl.desiredState.vel.z - maxVelDifference,
                   posControl.desiredState.vel.z + maxVelDifference);
  } else if (velTargetChange >= smallVelChange) {
    // Large and positive - acceleration is _up_. We are limited by
    // thrust/weight ratio which is usually about 2:1 (hover around 50%
    // throttle). T/W ratio = 2 means we are able to reach 1G acceleration in
    // "UP" direction. Hard limit to 0.5G to be on a safe side and avoid abrupt
    // throttle changes
    ...
    // Small - desired acceleration is less than 0.1G. We should be safe setting
    // velocity target directly - any platform should be able to satisfy this
    posControl.desiredState.vel.z = targetVel;
  }

  navDesiredVelocity[Z] =
      constrain(lrintf(posControl.desiredState.vel.z), -32678, 32767);
}

static void updateAltitudeThrottleController_MC(timeDelta_t deltaMicros) {
  // Calculate min and max throttle boundaries (to compensate for integral
  // windup)
  const int16_t thrCorrectionMin =
      getThrottleIdleValue() - currentBatteryProfile->nav.mc.hover_throttle;
  const int16_t thrCorrectionMax =
      getMaxThrottle() - currentBatteryProfile->nav.mc.hover_throttle;

  float velocity_controller =
      navPidApply2(&posControl.pids.vel[Z], posControl.desiredState.vel.z,
                   navGetCurrentActualPositionAndVelocity()->vel.z,
                   US2S(deltaMicros), thrCorrectionMin, thrCorrectionMax, 0);

  int16_t rcThrottleCorrection =
      pt1FilterApply4(&altholdThrottleFilterState, velocity_controller,
                      NAV_THROTTLE_CUTOFF_FREQENCY_HZ, US2S(deltaMicros));
  rcThrottleCorrection =
      constrain(rcThrottleCorrection, thrCorrectionMin, thrCorrectionMax);

  posControl.rcAdjustment[THROTTLE] = setDesiredThrottle(
      currentBatteryProfile->nav.mc.hover_throttle + rcThrottleCorrection,
      false);
}

bool adjustMulticopterAltitudeFromRCInput(void) {
  if (posControl.flags.isTerrainFollowEnabled) {
    const float altTarget = scaleRangef(
        rcCommand[THROTTLE], getThrottleIdleValue(), getMaxThrottle(), 0,
    ...
    const int16_t rcThrottleAdjustment =
        applyDeadbandRescaled(rcCommand[THROTTLE] - altHoldThrottleRCZero,
                              rcControlsConfig()->alt_hold_deadband, -500, 500);

    if (rcThrottleAdjustment) {
      /* Set velocity proportional to stick movement
       * Scale from altHoldThrottleRCZero to maxthrottle or minthrottle to
       * altHoldThrottleRCZero */

      // Calculate max up or min down limit value scaled for deadband
      int16_t limitValue =
          rcThrottleAdjustment > 0 ? getMaxThrottle() : getThrottleIdleValue();
      limitValue = applyDeadbandRescaled(limitValue - altHoldThrottleRCZero,
                                         rcControlsConfig()->alt_hold_deadband,
                                         -500, 500);

      int16_t rcClimbRate = ABS(rcThrottleAdjustment) *
                            navConfig()->mc.max_manual_climb_rate / limitValue;
      updateClimbRateToAltitudeController(rcClimbRate, 0, ROC_TO_ALT_CONSTANT);

      return true;
    } else {
      // Adjusting finished - reset desired position to stay exactly where pilot
      // released the stick
...
void setupMulticopterAltitudeController(void) {
  ...
  altHoldThrottleRCZero = constrain(
      altHoldThrottleRCZero,
      getThrottleIdleValue() + rcControlsConfig()->alt_hold_deadband + 10,
      getMaxThrottle() - rcControlsConfig()->alt_hold_deadband - 10);

  // Force AH controller to initialize althold integral for pending takeoff on
  // reset Signal for that is low throttle _and_ low actual altitude
  if (throttleIsLow &&
      fabsf(navGetCurrentActualPositionAndVelocity()->pos.z) <= 50.0f) {
    prepareForTakeoffOnReset = true;
  }
}

void resetMulticopterAltitudeController(void) {
  const navEstimatedPosVel_t *posToUse =
  ...
  posControl.desiredState.vel.z =
      posToUse->vel.z; // Gradually transition from current climb
  ...
  if (FLIGHT_MODE(FAILSAFE_MODE) || FLIGHT_MODE(NAV_RTH_MODE) ||
      FLIGHT_MODE(NAV_WP_MODE) || navigationIsExecutingAnEmergencyLanding()) {
  ...
  sqrtControllerInit(&alt_hold_sqrt_controller, posControl.pids.pos[Z].param.kP,
                     -fabsf(nav_speed_down), nav_speed_up, nav_accel_z);
}

static void applyMulticopterAltitudeController(timeUs_t currentTimeUs) {
  static timeUs_t previousTimePositionUpdate =
      0; // Occurs @ altitude sensor update rate (max
         // MAX_ALTITUDE_UPDATE_RATE_HZ)

  // If we have an update on vertical position data - update velocity and accel
  // targets
  if (posControl.flags.verticalPositionDataNew) {
    const timeDeltaLarge_t deltaMicrosPositionUpdate =
        currentTimeUs - previousTimePositionUpdate;
    previousTimePositionUpdate = currentTimeUs;

    // Check if last correction was not too long ago
    if (deltaMicrosPositionUpdate < MAX_POSITION_UPDATE_INTERVAL_US) {
      // If we are preparing for takeoff - start with lowset possible climb
      // rate, adjust alt target and make sure throttle doesn't jump
      if (prepareForTakeoffOnReset) {
        ...
        posControl.desiredState.pos.z =
            posToUse->pos.z - (navConfig()->mc.max_manual_climb_rate /
      ...
      // Position update has not occurred in time (first start or glitch), reset
      // altitude controller
...
bool adjustMulticopterHeadingFromRCInput(void) {
  if (ABS(rcCommand[YAW]) > rcControlsConfig()->pos_hold_deadband) {
    // Heading during Cruise Hold mode set by Nav function so no adjustment
    // required here
...
static void processMulticopterBrakingMode(const bool isAdjusting) {
  ...
  if (!(NAV_Status.state == MW_NAV_STATE_NONE ||
  ...
  const bool brakingEntryAllowed =
      IS_RC_MODE_ACTIVE(BOXBRAKING) && !STATE(NAV_CRUISE_BRAKING_LOCKED) &&
  ...
  /*
   * Case one, when we order to brake (sticks to the center) and we are moving
   * above threshold Speed is above 1m/s and sticks are centered Extra
   * condition: BRAKING flight mode has to be enabled
   */
  if (brakingEntryAllowed) {
     ...
     */
    setDesiredPosition(&navGetCurrentActualPositionAndVelocity()->pos, 0,
                       NAV_POS_UPDATE_XY);
    ...
    if (posControl.actualState.velXY >
        navConfig()->mc.braking_boost_speed_threshold) {
      ENABLE_STATE(NAV_CRUISE_BRAKING_BOOST);

      brakingBoostModeDisengageAt =
          millis() + navConfig()->mc.braking_boost_timeout;
    }
  }
  ...
  if (STATE(NAV_CRUISE_BRAKING_BOOST) &&
      (posControl.actualState.velXY <=
           navConfig()->mc.braking_boost_disengage_speed ||
       brakingBoostModeDisengageAt < millis())) {
    DISABLE_STATE(NAV_CRUISE_BRAKING_BOOST);
  }

  /*
   * Case when we were braking but copter finally stopped or we started to move
   * the sticks
   */
  if (STATE(NAV_CRUISE_BRAKING) &&
      (posControl.actualState.velXY <=
           navConfig()->mc.braking_disengage_speed || // We stopped
...
static bool adjustMulticopterCruiseSpeed(int16_t rcPitchAdjustment) {
  ...
  const float rcVelX = rcPitchAdjustment *
  ...
  if (rcVelX > posControl.cruise.multicopterSpeed) {
    ...
    posControl.cruise.multicopterSpeed +=
        MS2S(updateDeltaTimeMs) * rcVelX / 2.0f;
  } else {
    return false;
  }
  posControl.cruise.multicopterSpeed =
      constrainf(posControl.cruise.multicopterSpeed, 10.0f,
                 navConfig()->general.max_manual_speed);
...
bool adjustMulticopterPositionFromRCInput(int16_t rcPitchAdjustment,
  ...
  if (rcPitchAdjustment || rcRollAdjustment) {
    // If mode is GPS_CRUISE, move target position, otherwise POS controller
    // will passthru the RC input to ANGLE PID
    if (navConfig()->general.flags.user_control_mode == NAV_GPS_CRUISE) {
      const float rcVelX = rcPitchAdjustment *
                           navConfig()->general.max_manual_speed /
                           (float)(500 - rcControlsConfig()->pos_hold_deadband);
      const float rcVelY = rcRollAdjustment *
                           navConfig()->general.max_manual_speed /
                           (float)(500 - rcControlsConfig()->pos_hold_deadband);

      // Rotate these velocities from body frame to to earth frame
      const float neuVelX = rcVelX * posControl.actualState.cosYaw -
                            rcVelY * posControl.actualState.sinYaw;
      const float neuVelY = rcVelX * posControl.actualState.sinYaw +
                            rcVelY * posControl.actualState.cosYaw;

      // Calculate new position target, so Pos-to-Vel P-controller would yield
      // desired velocity
      posControl.desiredState.pos.x =
          navGetCurrentActualPositionAndVelocity()->pos.x +
          (neuVelX / posControl.pids.pos[X].param.kP);
      posControl.desiredState.pos.y =
          navGetCurrentActualPositionAndVelocity()->pos.y +
          (neuVelY / posControl.pids.pos[Y].param.kP);
    }
...
static float getVelocityHeadingAttenuationFactor(void) {
  ...
  if (navConfig()->mc.slowDownForTurning &&
      (navGetCurrentStateFlags() & NAV_AUTO_WP)) {
    const int32_t headingError = constrain(
        wrap_18000(posControl.desiredState.yaw - posControl.actualState.yaw),
        -9000, 9000);
...
static float getVelocityExpoAttenuationFactor(float velTotal, float velMax) {
  // Calculate factor of how velocity with applied expo is different from
  // unchanged velocity
  const float velScale = constrainf(velTotal / velMax, 0.01f, 1.0f);

  // navConfig()->max_speed * ((velScale * velScale * velScale) *
  // posControl.posResponseExpo + velScale * (1 - posControl.posResponseExpo)) /
  // velTotal;
  // ((velScale * velScale * velScale) * posControl.posResponseExpo + velScale *
  // (1 - posControl.posResponseExpo)) / velScale
  // ((velScale * velScale) * posControl.posResponseExpo + (1 -
  // posControl.posResponseExpo));
  return 1.0f - posControl.posResponseExpo *
                    (1.0f - (velScale * velScale)); // x^3 expo factor
}

static void updatePositionVelocityController_MC(const float maxSpeed) {
  if (FLIGHT_MODE(NAV_COURSE_HOLD_MODE)) {
    // Position held at cruise speeds below 0.5 m/s, otherwise desired neu
    // velocities set directly from cruise speed
    if (posControl.cruise.multicopterSpeed >= 50) {
      ...
      posControl.desiredState.vel.x =
          posControl.cruise.multicopterSpeed *
          cos_approx(CENTIDEGREES_TO_RADIANS(posControl.cruise.course));
      posControl.desiredState.vel.y =
          posControl.cruise.multicopterSpeed *
          sin_approx(CENTIDEGREES_TO_RADIANS(posControl.cruise.course));
  ...
  const float posErrorX = posControl.desiredState.pos.x -
                          navGetCurrentActualPositionAndVelocity()->pos.x;
  const float posErrorY = posControl.desiredState.pos.y -
                          navGetCurrentActualPositionAndVelocity()->pos.y;
   ...
   * 2 - in WP mission or RTH Trackback when: slowDownForTurning is OFF, not a
   * hold waypoint and computed speed is < maxSpeed
   */
  if (((navGetCurrentStateFlags() & NAV_AUTO_WP ||
        posControl.flags.rthTrackbackActive) &&
       !isNavHoldPositionActive() && neuVelTotal < maxSpeed &&
  ...
  // Apply expo & attenuation if heading in wrong direction - turn first,
  // accelerate later (effective only in WP mode)
  const float velHeadFactor = getVelocityHeadingAttenuationFactor();
  const float velExpoFactor =
      getVelocityExpoAttenuationFactor(neuVelTotal, maxSpeed);
...
static float computeNormalizedVelocity(const float value,
                                       const float maxValue) {
  return constrainf(scaleRangef(fabsf(value), 0, maxValue, 0.0f, 1.0f), 0.0f,
                    1.0f);
}

static float computeVelocityScale(const float value, const float maxValue,
  ...
  float scale = scaleRangef(normalized, attenuationStart, attenuationEnd, 0,
                            attenuationFactor);
  return constrainf(scale, 0, attenuationFactor);
}

static void updatePositionAccelController_MC(timeDelta_t deltaMicros,
                                             float maxAccelLimit,
  ...
  const float velErrorMagnitude =
      calc_length_pythagorean_2D(velErrorX, velErrorY);
  ...
  // Apply additional jerk limiting of 1700 cm/s^3 (~100 deg/s), almost any
  // copter should be able to achieve this rate This will assure that we wont't
  // saturate out LEVEL and RATE PID controller
  ...
  const float accelLimitXMin =
      constrainf(lastAccelTargetX - maxAccelChange, -accelLimitX, +accelLimitX);
  const float accelLimitXMax =
      constrainf(lastAccelTargetX + maxAccelChange, -accelLimitX, +accelLimitX);
  const float accelLimitYMin =
      constrainf(lastAccelTargetY - maxAccelChange, -accelLimitY, +accelLimitY);
  const float accelLimitYMax =
      constrainf(lastAccelTargetY + maxAccelChange, -accelLimitY, +accelLimitY);

  // TODO: Verify if we need jerk limiting after all

  /*
   * This PID controller has dynamic dTerm scale. It's less active when
   * controller is tracking setpoint at high speed. Full dTerm is required only
   * for position hold, acceleration and deceleration Scale down dTerm with 2D
   * speed
   */
  const float setpointScale = computeVelocityScale(
      setpointXY, maxSpeed, multicopterPosXyCoefficients.dTermAttenuation,
      multicopterPosXyCoefficients.dTermAttenuationStart,
      multicopterPosXyCoefficients.dTermAttenuationEnd);
  const float measurementScale =
      computeVelocityScale(posControl.actualState.velXY, maxSpeed,
  ...
  // Pre-calculated accelLimit and the logic of navPidApply2 function guarantee
  // that our newAccel won't exceed maxAccelLimit Thus we don't need to do
  // anything else with calculated acceleration
  float newAccelX =
      navPidApply3(&posControl.pids.vel[X], setpointX, measurementX,
                   US2S(deltaMicros), accelLimitXMin, accelLimitXMax,
                   0,         // Flags
                   1.0f,      // Total gain scale
                   dtermScale // Additional dTerm scale
      );
  float newAccelY =
      navPidApply3(&posControl.pids.vel[Y], setpointY, measurementY,
                   US2S(deltaMicros), accelLimitYMin, accelLimitYMax,
                   0,         // Flags
                   1.0f,      // Total gain scale
                   dtermScale // Additional dTerm scale
      );
  ...
  if (STATE(NAV_CRUISE_BRAKING_BOOST) &&
    ...
    const float boostFactor = constrainf(
        scaleRangef(posControl.actualState.velXY,
                    navConfig()->mc.braking_boost_speed_threshold,
                    navConfig()->general.max_manual_speed, 0.0f,
                    multicopterPosXyCoefficients.breakingBoostFactor),
        0.0f, multicopterPosXyCoefficients.breakingBoostFactor);
  ...
  const float accelForward = newAccelX * posControl.actualState.cosYaw +
                             newAccelY * posControl.actualState.sinYaw;
  const float accelRight = -newAccelX * posControl.actualState.sinYaw +
                           newAccelY * posControl.actualState.cosYaw;

  // Calculate banking angles
  const float desiredPitch = atan2_approx(accelForward, GRAVITY_CMSS);
  const float desiredRoll =
      atan2_approx(accelRight * cos_approx(desiredPitch), GRAVITY_CMSS);

  posControl.rcAdjustment[ROLL] = constrain(RADIANS_TO_DECIDEGREES(desiredRoll),
                                            -maxBankAngle, maxBankAngle);
  posControl.rcAdjustment[PITCH] = constrain(
      RADIANS_TO_DECIDEGREES(desiredPitch), -maxBankAngle, maxBankAngle);
}

static void applyMulticopterPositionController(timeUs_t currentTimeUs) {
  // Apply controller only if position source is valid. In absence of valid pos
  // sensor (GPS loss), we'd stick in forced ANGLE mode and pilots input would
  // be passed thru to PID controller
  ...
  // Passthrough rcCommand if adjusting position in GPS_ATTI mode except when
  // Course Hold active
  ...
  if (posControl.flags.horizontalPositionDataNew) {
    ...
    // If we have new position data - update velocity and acceleration
    // controllers
    if (deltaMicrosPositionUpdate < MAX_POSITION_UPDATE_INTERVAL_US) {
      // Get max speed for current NAV mode
      float maxSpeed = getActiveSpeed();
      updatePositionVelocityController_MC(maxSpeed);
      updatePositionAccelController_MC(deltaMicrosPositionUpdate,
                                       NAV_ACCELERATION_XY_MAX, maxSpeed);

      navDesiredVelocity[X] =
          constrain(lrintf(posControl.desiredState.vel.x), -32678, 32767);
      navDesiredVelocity[Y] =
          constrain(lrintf(posControl.desiredState.vel.y), -32678, 32767);
    } else {
      // Position update has not occurred in time (first start or glitch), reset
      // position controller
  ...
  rcCommand[PITCH] =
      pidAngleToRcCommand(posControl.rcAdjustment[PITCH],
                          pidProfile()->max_angle_inclination[FD_PITCH]);
  rcCommand[ROLL] =
      pidAngleToRcCommand(posControl.rcAdjustment[ROLL],
                          pidProfile()->max_angle_inclination[FD_ROLL]);
}

bool isMulticopterFlying(void) {
  bool throttleCondition =
...
static bool isLandingGbumpDetected(timeMs_t currentTimeMs) {
  /* Detection based on G bump at touchdown, falling Baro altitude and throttle
   * below hover. G bump trigger: > 2g then falling back below 1g in < 0.1s.
   * Baro trigger: rate must be -ve at initial trigger g and < -2 m/s when g
   * falls back below 1g Throttle trigger: must be below hover throttle with
   * lower threshold for manual throttle control */
  ...
  if (!gSpikeDetectTimeMs && acc.accADCf[Z] > 2.0f && baroAltRate < 0.0f) {
    ...
    if (currentTimeMs < gSpikeDetectTimeMs + 100) {
      if (acc.accADCf[Z] < 1.0f && baroAltRate < -200.0f) {
        ...
        const uint16_t hoverThrottleRange =
            currentBatteryProfile->nav.mc.hover_throttle - idleThrottle;
        return rcCommand[THROTTLE] <
               idleThrottle +
                   ((navigationInAutomaticThrottleMode() ? 0.8 : 0.5) *
                    hoverThrottleRange);
      }
...
bool isMulticopterCrashedInverted(timeMs_t currentTimeMs) {
  /* Disarms MR if inverted on the ground. Checks vertical velocity is low based
   * on Baro rate below 2 m/s */

  static timeMs_t startTime = 0;

  if ((ABS(attitude.values.roll) > 1000 || ABS(attitude.values.pitch) > 700) &&
      fabsf(baroAltRate) < 200.0f) {
    if (startTime == 0) {
      startTime = currentTimeMs;
    }

    /* Minimum 3s disarm delay + extra user set delay time (min overall delay of
     * 4s) */
    uint16_t disarmTimeDelay =
        3000 + S2MS(navConfig()->mc.inverted_crash_detection);
...
bool isMulticopterLandingDetected(void) {
  ...
  if (sensors(SENSOR_BARO)) {
    ...
    if (navConfig()->mc.inverted_crash_detection && !FLIGHT_MODE(TURTLE_MODE) &&
    ...
    bool gBumpDetectionUsable =
        navConfig()->general.flags.landing_bump_detection &&
  ...
  bool startCondition =
      (navGetCurrentStateFlags() & (NAV_CTL_LAND | NAV_CTL_EMERG)) ||
      (FLIGHT_MODE(FAILSAFE_MODE) && !FLIGHT_MODE(NAV_WP_MODE) &&
       !isMulticopterThrottleAboveMidHover()) ||
  ...
  bool velCondition = fabsf(navGetCurrentActualPositionAndVelocity()->vel.z) <
                          (MC_LAND_CHECK_VEL_Z_MOVING * sensitivity) &&
                      posControl.actualState.velXY <
                          (MC_LAND_CHECK_VEL_XY_MOVING * sensitivity);
  ...
  if (navGetCurrentStateFlags() & NAV_CTL_LAND) {
    // We have likely landed if throttle is 40 units below average descend
    // throttle We use rcCommandAdjustedThrottle to keep track of NAV corrected
    // throttle (isLandingDetected is executed from processRx() and rcCommand at
    // that moment holds rc input, not adjusted values from NAV core)
    ...
    if (!landingThrSamples) {
      if (currentTimeMs - landingDetectorStartedAt <
          S2MS(MC_LAND_THR_STABILISE_DELAY)) { // Wait for 1 second so throttle
                                               // has stabilized.
        return false;
      } else {
        landingDetectorStartedAt = currentTimeMs;
  ...
  if ((posControl.flags.estAglStatus == EST_TRUSTED) &&
    ...
    // TODO: Come up with a clever way to let sonar increase detection
    // performance, not just add extra safety.
    // TODO: Out of range sonar may give reading that looks like we landed, find
    // a way to check if sonar is healthy. surfaceMin is our ground reference.
    // If we are less than 5cm above the ground - we are likely landed
    possibleLandingDetected =
        possibleLandingDetected &&
        (posControl.actualState.agl.pos.z <=
         (posControl.actualState.surfaceMin + MC_LAND_SAFE_SURFACE));
  }
  ...
  if (possibleLandingDetected) {
     ...
     * Fixed time increased if Z velocity invalid to provide extra safety margin
     * against false triggers */
    const uint16_t safetyTime =
        posControl.flags.estAltStatus == EST_NONE ? 5000 : 1000;
    timeMs_t safetyTimeDelay =
        safetyTime + navConfig()->general.auto_disarm_delay;
...
bool isMulticopterThrottleAboveMidHover(void) {
  return rcCommand[THROTTLE] >
         0.5 * (currentBatteryProfile->nav.mc.hover_throttle +
...
static void applyMulticopterEmergencyLandingController(timeUs_t currentTimeUs) {
  ...
  if (posControl.flags.estAltStatus < EST_USABLE) {
    ...
    rcCommand[THROTTLE] =
        setDesiredThrottle(currentBatteryProfile->failsafe_throttle, true);
    return;
  }
  ...
  if (posControl.flags.verticalPositionDataNew) {
    ...
    // Check if last correction was not too long ago
    if (deltaMicrosPositionUpdate < MAX_POSITION_UPDATE_INTERVAL_US) {
      // target min descent rate at distance 2 x emerg descent rate above
      // takeoff altitude
      updateClimbRateToAltitudeController(
          0, 2.0f * navConfig()->general.emerg_descent_rate, ROC_TO_ALT_TARGET);
      updateAltitudeVelocityController_MC(deltaMicrosPositionUpdate);
      updateAltitudeThrottleController_MC(deltaMicrosPositionUpdate);
    } else {
      // due to some glitch position update has not occurred in time, reset
      // altitude controller
      resetMulticopterAltitudeController();
    }
...
void calculateMulticopterInitialHoldPosition(fpVector3_t *pos) {
  const float stoppingDistanceX =
      ...
      posControl.posDecelerationTime;
...
static void applyMulticopterHeadingController(void) {
  if (FLIGHT_MODE(NAV_COURSE_HOLD_MODE)) { // heading set by Nav during Course
                                           // Hold so disable yaw stick input
    rcCommand[YAW] = 0;
  }
...
void applyMulticopterNavigationController(
